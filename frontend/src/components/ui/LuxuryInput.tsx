...
export const LuxuryInput: React.FC<LuxuryInputProps> = ({ label, icon, className, ...props }) => {
  ...
  return (
    <div className={`relative group w-full mt-3 ${className || ''}`}>
      {/* Icon */}
      <div className={`absolute left-4 top-[55%] text-50xl -translate-y-1/2 transition-all duration-300 pointer-events-none z-20 mt-2  ${isFocused ? 'text-[#D4AF37]' : 'text-white/30 group-hover:text-white/50'}`}>
        {icon}
      </div>
